...
SELECT 
	...
	product_id,
	market_date,
  COUNT(product_id) OVER (PARTITION BY customer_id, product_id) AS total_purchases
FROM customer_purchases
ORDER BY customer_id, product_id, market_date ASC;	
...
SELECT 
  v.vendor_name,
  p.product_name,
  SUM(vi.product_price * 5) AS total_revenue
...
VALUES (9999, 'Test Product', '1 lb', 'unit', CURRENT_TIMESTAMP);					
